...
def generate_intention_embeddings():
    ...
    for intent in intentions:
        ...
        vectors = [np.array(f["embedding"], dtype=float) for f in faq_embeddings]

        # Calculer la moyenne
        moyenne = np.mean(vectors, axis=0)

        # 🔹 Convertir en liste avant de mettre à jour PostgreSQL
        moyenne_list = moyenne.tolist()

        cur.execute("""
            UPDATE chatbot.intention
            SET embedding = %s
            WHERE id_intent = %s
        """, (moyenne_list, id_intent))
